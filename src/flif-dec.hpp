...
bool flif_decode(IO& io, Images &images, flif_options &options, metadata_options &md) {
    return flif_decode(io, images, NULL, NULL, 0, images, options, md, NULL);
}

// With callback
template <typename IO>
bool flif_decode(IO& io, Images &images, callback_t callback, void *user_data, int initialProgressThreshold, Images &partial_images, flif_options &options, metadata_options &md) {
    return flif_decode(io, images, callback, user_data, initialProgressThreshold, partial_images, options, md, NULL);
}
